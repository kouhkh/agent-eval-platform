...
export function createTechnicalSpecRewriteRegressionAdapter(options = {}) {
  ...
  async function sourceChecks(value, root) {
    ...
    for (const file of value.sourceFiles) { const target = path.join(root, file.path); const bytes = await readFile(target).catch(() => null); const actual = bytes && hash(bytes); const matched = actual === file.sha256; checks.push({ checkId: `source:${file.id}`, label: `${file.label} SHA-256`, status: matched ? "pass" : "fail", expected: file.sha256, observed: actual || null, reasonCode: matched ? null : actual ? "SOURCE_FILE_HASH_MISMATCH" : "SOURCE_FILE_MISSING", evidenceRefs: [target] }); }
    return checks;
  }
  function asset(value) { return { id: ASSET_ID, title: "技术规格书语气改写 · 离线金标与白盒回归", purpose: "冻结飞书句级金标和装船机金标；只验证离线规则、路由、保护与审计。", project: "Planora 技术规格书改写", benchmarkKey: "technical-spec-rewrite-offline-goldens", assetState: sourceRoot ? "runnable" : "blocked", ...(sourceRoot ? {} : { blockedReason: "需配置 AGENT_EVAL_TECH_SPEC_REWRITE_ROOT；不会调用远端、3033或模型。" }), baseline: { sourceRevision: value.sourceRevision, fixtureSha256: value.fixtureSha256, sourceFiles: value.sourceFiles, fullDocumentSlots: value.fullDocumentSlots, fullDocumentEvidence: value.fullDocumentEvidence }, method: "校验版本和 SHA-256 后执行已检入离线测试；句法使用注入夹具。", basis: "冻结输入/期望/路由/保护/审计元数据；不以真实LLM作稳定pass。", objectiveChecks: ["版本和资产哈希", "离线改写/装船机金标", "路由", "白盒审计", "飞书金标"], proposedQualityChecks: [{ id: "customer-full-document-quality", status: "pending_confirmation", title: "7份客户全文等待人工确认的期望统计和白盒证据。" }], executor: { id: EXECUTOR_ID, argument: ASSET_ID } }; }
  return {
