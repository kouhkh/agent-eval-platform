...
const tools = [
  ...
  { name: "act", description: "执行浏览器动作；写动作必须携带 approvedScope。", inputSchema: { type: "object", required: ["sessionId", "action"], properties: { sessionId: { type: "string" }, action: { enum: ["click", "dblclick", "fill", "select", "check", "uncheck", "hover", "press", "scroll", "upload"] }, target: { type: ["object", "string"] }, value: {}, file: { type: "string" }, files: { type: "array", items: { type: "string" } }, approvedScope: { type: "string" }, dialogAction: { enum: ["accept", "dismiss"] }, dialogPromptText: { type: "string" }, waitFor: { type: "object", properties: { type: { enum: ["load", "url", "visible", "hidden", "attached", "detached", "text", "response"] }, url: { type: "string" }, method: { type: "string" } } }, deadlineMs: { type: "number" } } } },
